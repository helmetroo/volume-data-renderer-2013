...
gliReadTGA(FILE *fp, char *name)
{
  ...
  if (tgaHeader.colorMapType == 1) {
    ...
    colors = length + index;
    cmap = static_cast<GLubyte*>(malloc(colors * pelbytes));
  ...
  if (rle) {
    ...
    rleRec.laststate = 0;
    rleInfo = &rleRec;
    myfread = (int(*)(RLEstate*, unsigned char*, size_t, size_t, FILE*))rle_fread;
  } else {
    rleInfo = NULL;
    myfread = (int (*)(RLEstate*, unsigned char*, size_t, size_t, FILE*))(std_fread);
  }
